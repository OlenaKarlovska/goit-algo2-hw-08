...
def run_test():
    ...
    for query in queries:
        if query[0] == "Range":
            _, l, r = query
            range_sum_no_cache(array1, l, r)
        else:
            _, idx, val = query
            update_no_cache(array1, idx, val)
    ...
    for query in queries:
        if query[0] == "Range":
            _, l, r = query
            range_sum_with_cache(array2, l, r)
        else:
            _, idx, val = query
            update_with_cache(array2, idx, val)
    t_cache = time.perf_counter() - start
    speedup = t_no_cache / t_cache
    print(f"Без кешу : {t_no_cache:6.2f} c")
    print(f"LRU-кеш  : {t_cache:6.2f} c  (прискорення ×{speedup:.1f})")
